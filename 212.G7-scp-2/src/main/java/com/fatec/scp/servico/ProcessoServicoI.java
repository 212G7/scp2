...
//			}
			...
			if (e.getMessage().contains("could not execute statement")) {
				modelAndView.addObject("message", "Dados Inválidos - Processo já cadastrado.");
				logger.info(">>>>>> 5. Processo já cadastrado ==> " + e.getMessage());
			} else {
				modelAndView.addObject("message", "Erro não esperado - contate o administrador");
				logger.error(">>>>>> 5. erro não esperado ==> " + e.getMessage());
			}
		}
		return modelAndView;
	}
